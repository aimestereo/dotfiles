...
- **Fix atuin `Ctrl-R` not firing in fresh toolbox session (DOT-37 still in QA)** — the bash → xonsh exec transition (back in place after DOT-44) is suspected to cause a PTK rebuild that drops atuin's `Keys.ControlR` registration. DOT-43 had hoped to dodge this by making xonsh the login shell directly, but #908 made that infeasible. Diagnostic still needed: trace what PTK bindings hold on first `@events.on_ptk_create` fire vs after the first prompt; check whether atuin's init even ran (script error swallowed?).
