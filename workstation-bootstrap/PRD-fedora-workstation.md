...
- **Tier 2 — Toolbox `tools` (outer dev env):** ad-hoc shell work happens inside `toolbox enter tools` (manual; not auto-entered). A single `utils/install-personal-tools toolbox` script provisions all dev tools (mise, atuin, starship, claude, sd, git-delta, xonsh + xontribs). The toolbox↔host wrappers (`devpod`, `xdg-open`, `swaymsg`, `toolbox-run`) are stowed from `configs/toolbox/`, not installed by this script. xonsh is the interactive shell, replicating Mac zsh reflexes. `pass` lives here.
...
- **Hyper-chord app launchers** — `configs/sway/.config/sway/config.d/70-hyper-apps.conf` + `configs/sway/.local/bin/hyper-app-toggle`, mirroring the Mac `quad-mapping.lua` per-app switcher. Each `$hyper+<key>` binding (the `M-C-A-S` quad chord from keyd) drives the helper: switch to the app's named workspace and summon it (`flatpak run …`) when no matching window is on it, else `workspace back_and_forth` when the workspace is already focused with the app present. `for_window` rules pin each app to its workspace by `app_id`/`class`. Bindings: `a` Zen (browser), `c` GNOME Calendar (calendar), `i` Geary (mail), `m` Yandex Music (music), `o` Obsidian (obsidian), `s` Slack (slack), `z` Zoom (zoom). Three deliberate departures from the toggle pattern:
   ...
   - **`m` (Yandex Music)** is a Chromium `--app=https://music.yandex.ru --class=YandexMusic` web-app. Firefox-based Zen can't set a per-site `app_id` on Wayland, so a Chromium app window is the only way to give the web-app a distinct, workspace-bindable identity (omarchy uses the same Chromium-`--app` trick). Sole reason `org.chromium.Chromium` is installed.
...
- **The `~/.local/bin/devpod` wrapper** (`configs/toolbox/.local/bin/devpod`, stowed; DOT-55) — `flatpak-spawn --host /usr/local/bin/devpod` — bridges toolbox→host DevPod CLI. Load-bearing for Pattern B; requires `flatpak-xdg-utils` dnf-installed in toolbox.
