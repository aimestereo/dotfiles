...
- **Sway-side Hyper consumers** — DOT-59 corrected the keyd config syntax so CapsLock now emits the `M-C-A-S` quad chord; the Sway config still binds only `$mod+<key>` (Super alone). Net-new Hyper-keyed bindings (per-app launchers, window-management chords) will be added on demand against the now-working chord.
