...
- **The `~/.local/bin/devpod-up` wrapper** (`configs/toolbox/.local/bin/devpod-up`, stowed) is the **canonical bring-up command** for per-project containers (use it, not bare `devpod up`). It's **Pattern-B aware**: in the toolbox it re-execs itself on host via `flatpak-spawn --host` so loopback allocation edits the *host* `/etc/hosts` and `LOOPBACK_IP` is exported in the host-side devpod process. This re-exec is mandatory because `flatpak-spawn --host` **drops env vars** — a toolbox-side `export LOOPBACK_IP` + `devpod up` would never reach the host binary that substitutes `${localEnv:LOOPBACK_IP}`. Its helper `~/.local/bin/devpod-allocate-loopback-ip` (also stowed from `configs/toolbox/`) returns the project's registered `127.0.0.X` or allocates the next free one. The privileged `/etc/hosts` append goes through a narrow root-owned helper `/usr/local/sbin/devpod-hosts-register` (source `utils/devpod-hosts-register`, installed by `fedora-after-install`) exposed via a scoped `NOPASSWD` rule in `/etc/sudoers.d/devpod-loopback` — required because the `flatpak-spawn --host` re-exec has **no controlling TTY** for a `sudo` password prompt (DOT-62). The helper validates its `<project> <ip>` args and only ever writes the fixed `# devpod-loopback:` marker pair, so the grant is not a general `/etc/hosts` write primitive.
...
- **Sway WM bringup + theming** — base config + brightness keybindings + wallpaper + 15-min lock screen (swaylock + swayidle override via `current-background-image` symlink) are shipped (DOT-47); waybar (DOT-68), swayosd (DOT-71), the notification daemon (mako, theme-aware + do-not-disturb on Hyper+n, replaces dunst — DOT-71), and the application launcher (rofi on `$mod+d`, theme-aware — DOT-74) are shipped and wired to the theme pipeline. The launcher swap to walker was evaluated and declined: rofi can't consume the omarchy pipeline's rendered files (it only `@import`s `.rasi`, and a net-new `themed/rofi.rasi.tpl` is forbidden), so `theme-render` emits `rofi.rasi` from an inline heredoc instead — cheaper than walker, which would have dragged in the mandatory `elephant` backend daemon + autostart services.
