...
After push, monitor CI with these commands:

  gh pr checks                      # Check PR status
  gh run watch                      # Watch current run
  gh run view <run-id> --log-failed # Get failure details
...
Run pre-flight checks based on changed files, then report results.
After push, monitor CI and report any failures with analysis.
```
