...
│   ├── mako/          # Mako Wayland notification daemon (config includes the color-only ~/.config/theme/current/mako.ini; makoctl-reloaded by theme-set; do-not-disturb on Hyper+n). Fedora-only (excluded from Mac stow).
