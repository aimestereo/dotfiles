...
-- Refactor
--
vim.api.nvim_set_keymap('n', '<leader>cw', [[<cmd>let @/='\<'.expand('<cword>').'\>'<cr>"_ciw]], { desc = 'Rename word' })
vim.api.nvim_set_keymap('x', '<leader>cw', [[y<cmd>let @/=substitute(escape(@", '/'), '\n', '\\n', 'g')<cr>"_cgn]], { desc = 'Rename selected word' })
...
vim.keymap.set('n', '<C-f>', '<cmd>silent !tmux neww tmux-sessionizer<CR>', { desc = 'Tmux sessionizer: [f]ind project' })
